...
class FaceRecog(Thread):
    ...
    def run(self):
        ...
        self.final_result = self.extractResult(self.recog_result)
        # 이렇게 하면 (성별, 연령대)튜플을 뱉는데
        # 이 튜플을 서버에 뱉어내서
        # 서버가 쿼리문 돌리고
        # 디스플레이 쓰레드가 클라이언트에게 전달할 방법을 찾아야겠음
        # 전역변수 쓰면 어때요?
    ...
    def extractResult(self, recog_result):
        ...
        if max_index == 0:
            final_result = ('male', 10)
        elif max_index == 1:
            final_result = ('male', 20)
        elif max_index == 2:
            final_result = ('male', 30)
        elif max_index == 3:
            final_result = ('male', 40)
        elif max_index == 4:
            final_result = ('male', 50)
        elif max_index == 5:
            final_result = ('male', 60)
        elif max_index == 6:
            final_result = ('female', 10)
        elif max_index == 7:
            final_result = ('female', 20)
        elif max_index == 8:
            final_result = ('female', 30)
        elif max_index == 9:
            final_result = ('female', 40)
        elif max_index == 10:
            final_result = ('female', 50)
        elif max_index == 11:
            final_result = ('female', 60)
